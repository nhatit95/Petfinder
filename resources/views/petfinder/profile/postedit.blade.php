            ...
            <div class="col-lg-12 col-md-12">
                <div class="">
                    ...
                    <div class="content">
                        ...
                        <form role="form" action="{{ route('petfinder.profile.postedit', ['id' => $arPosts->post_id]) }}" method="post" enctype="multipart/form-data">
                            {{csrf_field()}}
                            <div class="box-body">

                                <div class="form-group">
                                  <label for="">Pet name</label>
                                  <input type="text" class="form-control" required="true" name="post_name" id="post_name" value="{{$arPosts->post_name}}">
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2" name="cat_id" id="cat_id" required="true" style="width: 100%;">
                                      @foreach($arPostCats as $arPostCat)
                                        @if($arPosts->cat_id == $arPostCat->cat_id)
                                            <option selected="selected" value="{{$arPostCat->cat_id}}">{{$arPostCat->cat_name}}</option>
                                        @else
                                            <option value="{{$arPostCat->cat_id}}">{{$arPostCat->cat_name}}</option>
                                        @endif
                                      @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="form-group">
                                  ...
                                  <input type="text" class="form-control" required="true" name="discription" id="discription" value="{{$arPosts->discription}}">
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <img src="{{ asset('storage/app/files/'.$arPosts->avatar) }}" name="oldPicture" width="30%" height="30%">
                                </div>

                                <div class="form-group">
                                  <label for="">Status</label>
                                  <input type="text" class="form-control" required="true" name="status" id="status" value="{{$arPosts->status}}">
                                </div>

                                <div class="form-group">
                                    <label>Contents</label>
                                    <textarea class="ckeditor" id="content" required="true" rows="5" name="content">{{$arPosts->content}}</textarea>
                                    <script>
